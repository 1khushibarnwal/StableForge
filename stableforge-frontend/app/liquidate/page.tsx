...
import { useAccount, usePublicClient, useReadContracts, useWriteContract, useWaitForTransactionReceipt } from 'wagmi';
...
export default function LiquidatePage() {
  ...
  const [collateralToken, setCollateralToken] = useState<CollateralToken>(COLLATERAL_TOKENS[0]);
  ...
  const needsApproval = allowance !== undefined && debtBn > 0n && allowance < debtBn;
  ...
  const sorted = [...positions]
    .filter(p => filterAtRisk ? parseFloat(formatUnits(p.healthFactor, 18)) < 1.5 && p.totalDscMinted > 0n : true)
    .sort((a, b) => {
      const aVal = sortBy === 'hf' ? Number(a.healthFactor) : Number(a.totalDscMinted);
      const bVal = sortBy === 'hf' ? Number(b.healthFactor) : Number(b.totalDscMinted);
  ...
  return (
    <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '40px 24px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: selectedPosition ? '1fr 380px' : '1fr', gap: '24px' }}>
        {/* Table */}
        <div className="card" style={{ overflow: 'hidden' }}>
          {loading ? (
            <div style={{ padding: '60px', textAlign: 'center', color: 'var(--text-muted)' }}>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: '13px' }}>Scanning on-chain events…</div>
              <div style={{ marginTop: '12px', display: 'flex', gap: '8px', justifyContent: 'center' }}>
                {[1,2,3].map(i => (
                  <div key={i} className="shimmer" style={{ width: '8px', height: '8px', borderRadius: '50%', animationDelay: `${i * 0.2}s` }} />
                ))}
              </div>
            </div>
          ) : sorted.length === 0 ? (
              ...
              <thead>
                <tr>
                  ...
                  <th style={{ cursor: 'pointer' }} onClick={() => handleSort('debt')}>
                    <span style={{ display: 'inline-flex', alignItems: 'center' }}>SFC Debt <SortIcon col="debt" /></span>
                  </th>
                ...
                {sorted.map(pos => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td>
                        {(status === 'danger' || (status === 'warning' && parseFloat(formatHealthFactor(pos.healthFactor)) < 1.0)) ? (
                          <button
                            className="btn-secondary"
                            style={{ fontSize: '11px', padding: '5px 12px', background: 'var(--danger-bg)', color: 'var(--danger)', borderColor: 'rgba(248,113,113,0.3)' }}
                            onClick={e => { e.stopPropagation(); setSelectedPosition(pos); }}
                          >
                            Liquidate
                          </button>
                        ) : (
                          <span style={{ fontSize: '11px', color: 'var(--text-muted)' }}>Safe</span>
                        )}
                      </td>
                    </tr>
                  );
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div className="card" style={{ overflow: 'hidden' }}>
              <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)' }}>
                <div style={{ fontSize: '11px', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: '4px' }}>
                  Selected Position
                </div>
              ...
              <div style={{ padding: '16px 20px' }}>
                {[
                  { label: 'Health Factor', value: <HealthFactorBadge hf={selectedPosition.healthFactor} /> },
                  { label: 'SFC Debt', value: `${formatSfc(selectedPosition.totalDscMinted)} SFC` },
                  { label: 'Collateral Value', value: formatUsd(selectedPosition.collateralValueInUsd) },
                  { label: 'WETH Deposited', value: `${formatUnits(selectedPosition.wethBalance, 18).slice(0,8)} WETH` },
                  { label: 'WBTC Deposited', value: `${formatUnits(selectedPosition.wbtcBalance, 8).slice(0,8)} WBTC` },
                ].map(({ label, value }) => (
                  <div key={label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid var(--border)', fontSize: '12px' }}>
                    ...
                    <span style={{ fontFamily: typeof value === 'string' ? 'var(--font-mono)' : undefined, color: 'var(--text-primary)' }}>{value}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="card" style={{ padding: '20px' }}>
              ...
              <div style={{ marginBottom: '14px' }}>
                ...
                <select
                  ...
                  onChange={e => setCollateralToken(COLLATERAL_TOKENS.find(t => t.symbol === e.target.value) || COLLATERAL_TOKENS[0])}
                >
              ...
              <div style={{ marginBottom: '16px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                  ...
                  <button
                    onClick={() => setDebtToCover((Number(selectedPosition.totalDscMinted) / 1e18).toString())}
              ...
              <div className="alert alert-info" style={{ marginBottom: '16px', fontSize: '12px' }}>
                <span>🎯</span>
                <span>You receive {collateralToken.symbol} worth <strong>110%</strong> of the debt you cover.</span>
              </div>

              <TxStatus status={txStatus} hash={hash} successMessage="Liquidation successful!" />
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: txStatus !== 'idle' ? '12px' : '0' }}>
                ...
                <button
                  ...
                  disabled={!debtToCover || needsApproval || isPending || isConfirming}
                  style={{ width: '100%', justifyContent: 'center', background: 'var(--danger)' }}
                >
                  {isPending || isConfirming ? 'Liquidating…' : 'Liquidate Position'}
                </button>
              </div>
            </div>
          </div>
